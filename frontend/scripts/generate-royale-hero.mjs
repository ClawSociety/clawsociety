...
/**
 * generate-royale-hero.mjs
 *
 * Generates an Agent Royale hero image using DALL-E 3.
 *
 * Usage:
 *   node scripts/generate-royale-hero.mjs
 *
...
const OUTPUT_FILE = path.join(OUTPUT_DIR, "royale-hero.png");

const PROMPT =
  "Dark futuristic battle arena, top-down perspective. Glowing autonomous AI agents and combat drones battling in a shrinking hexagonal zone. Purple and orange neon lighting, explosions, particle effects, volumetric fog. Dark background with intense action. No text, no logos, no UI elements. Cinematic, dramatic, game art style.";
...
/**
 * Create a minimal dark placeholder PNG (gradient from dark orange to black).
 */
function createPlaceholderPng(width, height) {
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < width; x++) {
      const xt = x / width;
      // Dark with subtle orange/purple hints
      raw[offset++] = Math.round(15 + 20 * (1 - t) * xt); // R (orange)
      raw[offset++] = Math.round(5 + 8 * (1 - t) * xt); // G
      raw[offset++] = Math.round(10 + 15 * (1 - t) * (1 - xt)); // B (purple)
      raw[offset++] = 255; // A
    }
  }
...
async function main() {
  console.log("==============================================");
  console.log(" Agent Royale Hero Image Generator (DALL-E 3)");
  console.log("==============================================");
  ...
  try {
    console.log("Generating Agent Royale hero image...");
